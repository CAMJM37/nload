...
OutputStats::OutputStats(AppControl* appControl, const vector<Device*>& devices, const string& fileName)
    ...
    for(vector<Device*>::const_iterator itDevice = m_devices.begin(); itDevice != m_devices.end(); ++itDevice)
    {
        ...
        if(device->exists())
        {
            ...
            (*m_stream) << formatTimeStamp(timeStampSeconds, timeStampMicroseconds) << ' '
                        << device->getName() << ' '
                        << "CurIn: " + formatTrafficValue(statistics.getDataInPerSecond(), 2) << ' '
                        << "CurOut: " + formatTrafficValue(statistics.getDataOutPerSecond(), 2) << ' '
...
string OutputStats::formatTimeStamp(unsigned long seconds, unsigned long microseconds)
{
...
string OutputStats::formatTrafficValue(unsigned long value, int precision)
{
